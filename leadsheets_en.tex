...
\begin{options}
  ...
  \keybool{bar-shortcuts}\Default{false}
    Makes the characters \code{:} and \code{|} active inside the \env{song}
    environment.  See sections~\ref{sec:special-characters}
...
catecory codes:
\begin{labeling}[--]{xxxxx}
  \item[\code{\textasciicircum}] category code 13 (active). It is a
    shortcut for the \cs{chord} command.
  \item[\code{\_}] category code 13 (active). It is a shortcut for the
    \cs{writechord} command.
  \item[\code{|}] category code 13 (active).  Used for typesetting bars.
  \item[\code{:}] category code 13 (active).  Used for typesetting bars.
  \item[\code{\#}] category code 12 (other).  Used fo chord names.
\end{labeling}

Actually the characters \code{|} and \code{:} are \emph{not} changed per
default.  In order to do that you have to use the option
\option{bar-shortcuts}.

For details on the usage of the characters \code{|} and \code{:} see
section~\ref{sec:typesetting-bars}.  The usage of chords is explained in
section~\ref{sec:placing-chords}.
...
Inside the \env{song} environment the characters \code{\textasciicircum} and
\code{\_} are active characters\footnote{There are more characters with a
  special function, see
  section~\ref{sec:special-characters}}. \code{\textasciicircum} is a shortcut
for the command \cs{chord}, \code{\_} is a shortcut for \cs{writechord}.
Those commands have the following functions:
...
There's also a general option for setting the default of the
\option{after-label} for all environments:
\begin{options}
...
the characters \code{:} nor \code{|}. Per default they are not active, though.
If you want to use the shortcut syntax you have to use the option
\option{bar-shortcuts}.  Here's a short example that emulates the behaviour by
setting the characters active explicitly:
...
\begin{labeling}[--]{xxxxx}
  \item[\code{|}] \cs{normalbar}\cs*{space} (the space is there because
    otherwise it eats following spaces which would be annoying)
...
\begin{description}
  \item[minimal] This only typesettes the song title in a \cs{section}\sarg.
  \item[tabular] This typesets the song title in a \cs{section} and prints
    some song properties in a \env{tabular} below it.  This template needs the
    \pkg{array}~\cite{pkg:array} package loaded.
\end{description}

\subsection{Own Templates}
\subsubsection{The Principle and Available Commands}\label{sec:available-commands}
...
In order to give you an idea on how to use templates I'll show you how the
existing ones are defined and one new definition.

\paragraph{The \enquote{minimal} template}
...
as the \enquote{minimal} template.
...
\paragraph{The \enquote{tabular} template}
    ...
    \begin{tabular}{
      ...
      \ifsongproperty{key}
        {%
          ...
          \expandsongpropertycode{\writechord{\songproperty{key}}} \\%
        }
        {}%
    \end{tabular}
    \par\endgroup
...
A song using the \enquote{tabular} template:
