...
\begin{commands}
  ...
  \command{writechord}[\marg{chord}]
    ...
    \option{transpose} and \option{enharmonic} befor printing it.  This
    command can/should be used for placing chords inline or for typesetting
    the \property{key} property in a template.
\end{commands}
...
\subsubsection{Caveat}\label{sec:placing-chords:caveat}
...
nothing (but the spaces around it are of course there).
...
\begin{table}
  ...
  \newcommand*\showtranslation[1]{%
    \texttt{leadsheets/#1} &
    \GetTranslationForWarn{English}{leadsheets/#1} &
    \GetTranslationForWarn{German}{leadsheets/#1}
  }
  \begin{tabular}{lll}
    \toprule
