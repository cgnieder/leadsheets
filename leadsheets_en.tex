...
\chordname{B} and \chordname{Bb} in German (and a few other European
...
\begin{options}
  ...
  \keychoice{output-notation}{german,english}\Default{english}
    \setchordnames{output-notation=german}If\changedversion{0.2} set to
    \code{german} B is output as \chordname{B} and B-flat as \chordname{Bb}.
  ...
  \keyval{german-H}{code}\Default{H}
    ...
    \keyis{output-notation}{german}.
\end{options}
% TODO: reformulate the next paragraphs:
...
\begin{options}
  ...
  \keyval{class}{class-name}\Default{default}
    The \env{verse} environment and all related environments all belong to a
    class, the default class is called \code{default}.  This is of interest
    when using the \option{remember-chords} and \option{recall-chords}
    options.\label{option:class}
\end{options}
